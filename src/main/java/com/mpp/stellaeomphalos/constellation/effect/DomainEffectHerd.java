...
public final class DomainEffectHerd extends DomainEntityScan<Animal> {
    ...
    /** The grazable registry: animal type -> shed produce. */
    private static final Map<EntityType<?>, Item> GRAZEABLE = Map.of(
            EntityType.SHEEP, Items.WHITE_WOOL,
            EntityType.COW, Items.LEATHER,
            EntityType.CHICKEN, Items.FEATHER,
            EntityType.PIG, Items.PORKCHOP,
            EntityType.RABBIT, Items.RABBIT_HIDE);

    public DomainEffectHerd(@Nullable MajorSign owner) {
        super(owner, Animal.class, MAX_TARGETS);
        setSearchFilter(GRAZEABLE.keySet()::contains);
    }
    ...
    public boolean play(DomainContext ctx, float strength, DomainProperties props) {
        ...
        for (var animal : collect(level, ctx.origin(), props.size())) {
            ...
            if (animal.isBaby() || level.random.nextFloat() >= DROP_CHANCE * props.potency()) continue;
            var produce = GRAZEABLE.get(animal.getType());
            if (produce == null) continue;
            var stack = new ItemStack(produce);
            animal.spawnAtLocation(stack);
            drops++;
            DomainParticles.broadcast(level, animal.blockPosition(), 96.0, PktDomainParticle.Types.HERD,
    ...
    private boolean playCorrupted(DomainContext ctx, DomainProperties props) {
        ...
        var holder = level.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE)
                ...
                .orElse(level.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE)
                        .getHolderOrThrow(net.minecraft.world.damagesource.DamageTypes.MAGIC));
        var source = new DamageSource(holder);
        boolean acted = false;
        for (var animal : collect(level, ctx.origin(), props.size()))
            acted |= DamageKit.apply(animal, source, (float) (CORRUPTED_DAMAGE * Math.max(props.potency(), 1.0)));
        return acted;
    }
}
