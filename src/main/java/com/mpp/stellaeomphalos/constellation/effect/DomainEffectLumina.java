...
/**
 * lucerna — lumina. Status-type effect: no strength consumption. Every 80 ticks renews a very
 * large spawn-deny token (radius 64 + mirrors x 64; token cap 400 ticks, rebuilt on radius change).
...
public final class DomainEffectLumina extends DomainEffect implements DomainEffectStatus {
    ...
    public DomainEffectLumina(@Nullable MajorSign owner) { super(owner); }
    ...
    public boolean play(DomainContext ctx, float strength, DomainProperties props) {
        var level = ctx.level();
        if (props.corrupted()) {
            // Corrupted: rewind night time (doDaylightCycle honored — nothing to rewind when frozen).
            if (level.isNight() && level.getGameRules().getBoolean(net.minecraft.world.level.GameRules.RULE_DAYLIGHT)) {
