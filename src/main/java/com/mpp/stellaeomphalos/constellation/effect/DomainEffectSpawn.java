...
public final class DomainEffectSpawn extends DomainPositionCache<SpawnEntry> {
    ...
    private static final Map<EntityType<?>, EntityType<?>> CORRUPTED_REPLACEMENTS =
            Map.of(
    ...
    public boolean play(DomainContext ctx, float strength, DomainProperties props) {
        ...
        if (com.mpp.stellaeomphalos.constellation.domain.DomainWorkBudget.entities(
                                level,
                                Entity.class,
                                crowdBox,
                                entity ->
        ...
        for (var entry : entries()) {
            if (entry.increment() < WARMUP_TICKS) {
                if (DomainParticles.intervalPassed(
                        level,
                        entry.pos(),
                        PktDomainParticle.Types.SPAWN,
                        WARMUP_PARTICLE_INTERVAL))
