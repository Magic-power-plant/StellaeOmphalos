...
/**
 * 《方块物品实体完整清单》粒子类型的声明类。本类**只负责服务端/注册表侧的登记**：客户端渲染
 * （provider 绑定、贴图、顶点色）属于《客户端渲染界面与音效》工作流，因此这里不得出现任何 client 包引用。
 *
 * <p>{@code floating_cube} 使用 {@code new SimpleParticleType(true)}：它的贴图由运行时
 * 从流体材质推导（"总是显示"），其余 6 个使用 {@code false}（由调用方按需触发）。
 */
public final class ParticleContent {
