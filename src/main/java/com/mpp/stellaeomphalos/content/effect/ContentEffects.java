...
import com.mpp.stellaeomphalos.content.enchantment.EnchantmentContent;
import com.mpp.stellaeomphalos.content.particle.ParticleContent;
import com.mpp.stellaeomphalos.content.particle.SoundContent;
...
public final class ContentEffects {
    ...
    /** 幂等装配：声明其余《方块物品实体完整清单》内容并挂接 Forge 事件监听器。 */
    public static void attach(IEventBus modBus) {
        if (!ATTACHED.compareAndSet(false, true)) return;
        EnchantmentContent.initialize();
        ParticleContent.initialize();
        ...
        com.mpp.stellaeomphalos.knowledge.advancement.AdvancementTriggers.initialize();
        ...
        forge.addListener(ContentEffects::onLethalDamage);
        forge.addListener(ContentEffects::onDeath);
        forge.addListener(ContentEffects::onDrops);
        forge.addListener(ContentEffects::onLivingTick);
        forge.addListener(ContentEffects::onPlayerTick);
        forge.addListener(ContentEffects::onLogout);
        // 《方块物品实体完整清单》§6.3.5：护符修正经 M-1/M-2 的 GameplayQueries 桥注入原版附魔查询。
        forge.addListener(
    ...
    private static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        var equipped = EnchantmentContent.equippedTickEnchantments();
