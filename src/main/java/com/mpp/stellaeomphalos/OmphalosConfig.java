...
public final class OmphalosConfig {
    public enum Retrogen { OFF, LOADED_ONLY, NEW_CHUNKS }
    ...
    static {
        ...
        COMMON.values.put("worldgen.retrogen", COMMON.builder.defineEnum("worldgen.retrogen", Retrogen.OFF));
        ...
        SERVER.values.put("gameplay.lumenFluidHotInteraction", SERVER.builder.define("gameplay.lumenFluidHotInteraction", "sand_and_rare"));
        SERVER.values.put("gameplay.lumenFluidColdResultBlock", SERVER.builder.define("gameplay.lumenFluidColdResultBlock", "minecraft:ice"));
        SERVER.values.put("gameplay.lumenFluidHotResultBlock", SERVER.builder.define("gameplay.lumenFluidHotResultBlock", "minecraft:sand"));
        ...
        COMMON.build(); SERVER.build(); CLIENT.build();
    }
    ...
    private static void onLoading(ModConfigEvent.Loading event) { refresh(event); }
    private static void onReloading(ModConfigEvent.Reloading event) { refresh(event); }
    ...
    public static int serverRevision() { return SERVER_REVISION.get(); }
    ...
    public static final class Section {
        ...
        private Section(ModConfig.Type type) { this.type = type; }
        private void bool(String key, boolean value) { values.put(key, builder.define(key, value)); }
        private void integer(String key, int value, int min, int max) { values.put(key, builder.defineInRange(key, value, min, max)); }
        private void decimal(String key, double value, double min, double max) { values.put(key, builder.defineInRange(key, value, min, max)); }
        private void build() { spec = builder.build(); defaults(); }
        ...
        public java.util.Set<String> keys() { return java.util.Set.copyOf(values.keySet()); }
        public ForgeConfigSpec spec() { return spec; }
        public boolean flag(String key) { return (Boolean) snapshot.get(key); }
        public int integer(String key) { return ((Number) snapshot.get(key)).intValue(); }
        public double decimal(String key) { return ((Number) snapshot.get(key)).doubleValue(); }
        ...
        public void toggle(String key) {
            if (!(values.get(key) instanceof ForgeConfigSpec.BooleanValue value)) throw new IllegalArgumentException(key);
            value.set(!value.get());
            spec.save();
            refresh();
        }
