...
import com.mpp.stellaeomphalos.core.util.world.BlockChangeBus;
...
public final class BlockChangeDispatcher {
    ...
    public static void broken(BlockEvent.BreakEvent event) {
        if (event.getLevel() instanceof ServerLevel level)
            enqueue(level, event.getPos(), event.getState(), BlockChangeBus.ChangeSource.PLAYER);
    }
    @SubscribeEvent(priority = EventPriority.LOWEST)
    public static void placed(BlockEvent.EntityPlaceEvent event) {
        ...
        if (event instanceof BlockEvent.EntityMultiPlaceEvent multiple) {
            multiple.getReplacedBlockSnapshots().forEach(snapshot -> enqueue(level, snapshot.getPos(),
                    snapshot.getReplacedBlock(), BlockChangeBus.ChangeSource.ENTITY));
        } else enqueue(level, event.getPos(), event.getBlockSnapshot().getReplacedBlock(), BlockChangeBus.ChangeSource.ENTITY);
    }
    private static void enqueue(ServerLevel level, BlockPos pos, BlockState before, BlockChangeBus.ChangeSource source) {
        ...
        RuntimeServices.current().scheduler().schedule(0, () -> {
            ...
            if (!before.equals(after)) {
                MinecraftForge.EVENT_BUS.post(new BlockChangeNotice(address, before, after));
                BlockChangeBus.publish(current, address.position(), before, after, source);
            }
        });
    }
}
