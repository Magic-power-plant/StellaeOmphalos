...
/**
 * 进度触发器 {@code attune_crystal}：水晶物品被调谐到指定星象时触发。
 * 条件中的 {@code sign} 字段保存星象 id；缺省表示"任意星象"。
 */
public final class AttuneCrystalTrigger extends SimpleGsonTrigger<AttuneCrystalTrigger.Instance> {

    public AttuneCrystalTrigger() {
        super(new ResourceLocation(Omphalos.MODID, "attune_crystal"),
    ...
    public static final class Instance extends SubjectInstance {
        ...
        private Instance(ContextAwarePredicate player, Optional<ResourceLocation> sign, JsonObject original) {
            super(new ResourceLocation(Omphalos.MODID, "attune_crystal"), player, sign, original);
        }
