...
public final class OmphalosClient {
    ...
    private static ClientTickScheduler scheduler() {
        return new ClientTickScheduler(
                error -> LogUtils.getLogger().error("Client task failed", error));
    }
        ...
        event.enqueueWork(
                () -> {
                    ...
                    ClientSessionCleaner.register(
                            "fragment_assembler", OmphalosChannel::resetClient);
                    ClientSessionCleaner.register(
                            "client_tasks",
                            () -> {
                                scheduler.close();
                    ...
                    com.mpp.stellaeomphalos.client.codex.ClientKnowledgeCache.attach();
    ...
    private static void tick(TickEvent.ClientTickEvent event) {
        ...
        if (!announced) {
            announced = true;
            MinecraftForge.EVENT_BUS.post(new ClientReadyEvent());
        }
    ...
    private static void receive(OmphalosPayload payload) {
        if (payload instanceof PktSyncGateOpen packet)
            GATE.begin(ProtocolVersion.parse(packet.protocol()), System.nanoTime());
        else if (payload instanceof PktSyncGateClose) {
            GATE.reset();
            MIRRORS.clear();
        } else if (payload instanceof PktSyncDataset packet) {
            if (!MIRRORS.apply(packet))
                sendDependent(
                        new PktDatasetRequest(packet.dataset(), MIRRORS.version(packet.dataset())));
        } else if (payload instanceof PktConfigVersion packet) configVersion = packet.version();
        else if (payload instanceof PktThrottleNotice packet)
            LogUtils.getLogger()
                    .debug(
                            "Throttled payload {}: {} dropped",
                            packet.payloadId(),
                            packet.dropped());
        else if (payload instanceof PktNetworkDebugDump packet) {
            if (packet.data().getBoolean("SessionStarted"))
                debugUntil = scheduler.currentTick() + 400;
            else
            ...
            packet.issues()
                    .forEach(
                            issue ->
                                    chat.addMessage(
                                            net.minecraft.network.chat.Component.literal(issue)));
        } else if (!HANDLERS.dispatch(Minecraft.getInstance(), payload))
            throw new IllegalArgumentException("No client payload handler");
    }

    public static boolean sendDependent(OmphalosPayload payload) {
        if (!GATE.allows(OmphalosChannel.PAYLOADS.type(payload).minimumMinor(), System.nanoTime()))
            return false;
        OmphalosChannel.sendToServer(payload);
        return true;
    }

    private static void debugClick(
        ...
        if (event.getLevel().isClientSide
                ...
                && debugUntil > scheduler.currentTick())
            sendDependent(new PktNetworkDebugDumpRequest(event.getPos()));
    }

    public static MirrorDataHub mirrors() {
        return MIRRORS;
    }

    public static PayloadHandlers<Minecraft> handlers() {
        return HANDLERS;
    }
