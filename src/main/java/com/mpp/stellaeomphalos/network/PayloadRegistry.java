...
public final class PayloadRegistry {
    ...
    public record Type<T extends OmphalosPayload>(
            int id, Direction direction, Class<T> type, Codec<T> codec, int minimumMinor) {}
    ...
    public <T extends OmphalosPayload> void register(
            int id, Direction direction, Class<T> type, Codec<T> codec, int minimumMinor) {
        if (frozen || id <= lastId || types.containsKey(type))
            throw new IllegalStateException("Duplicate, unordered or late payload " + id);
    ...
    public byte[] encode(OmphalosPayload payload) {
        ...
        if ((payload instanceof com.mpp.stellaeomphalos.network.toClient.PreviewDiffPayload
                        || payload
                                instanceof
                                com.mpp.stellaeomphalos.network.toClient.PreviewStartPayload)
                && bytes.length > 8192) throw new IllegalArgumentException("Preview exceeds 8 KiB");
        if (bytes.length > ChunkedEnvelope.MAX_BYTES)
        ...
        try {
            String json =
                    StandardCharsets.UTF_8
                            ...
                            .toString();
            return type.codec()
                    .parse(JsonOps.INSTANCE, BoundedJson.parse(new java.io.StringReader(json)))
                    .getOrThrow(
                            false,
                            message -> {
                                throw new IllegalArgumentException(message);
