...
/** 《方块物品实体完整清单》§6.2.1 注册总账与 §6.3.2 / §6.3.4 行为的运行期验收（AC-6.1 / AC-6.2 / AC-6.3）。 */
@GameTestHolder(Omphalos.MODID)
@PrefixGameTestTemplate(false)
public final class CatalogBlocksGameTests {
    ...
    public static void part_six_menus_are_registered_and_validate_the_station(GameTestHelper helper) {
        for (String id : com.mpp.stellaeomphalos.content.menu.StationMenus.ids())
            helper.assertTrue(
        ...
        // 方块缺失时容器必须判定失效。
        helper.assertTrue(
                !com.mpp.stellaeomphalos.content.menu.StationMenus.stillValid(
