...
public final class StationInteractionGameTests {
    ...
    public static void chart_draw_consumes_one_paper_and_never_overwrites_glass(GameTestHelper h) {
        ...
        level.setBlock(pos, MachineContent.STAR_CHART_TABLE.get().defaultBlockState(), 3);
        var machine = (MachineContent.MachineBlockEntity) level.getBlockEntity(pos);
        ...
        data.putInt("Paper", 2);
        machine.load(data);
        machine.inventory().setStackInSlot(1, new ItemStack(CatalogItems.STAR_GLASS.get()));
    ...
    public static void telescope_rotation_has_eight_static_states(GameTestHelper h) {
        var pos = h.absolutePos(new BlockPos(1, 1, 1));
        var level = h.getLevel();
        level.setBlock(pos, MachineContent.SPYGLASS.get().defaultBlockState(), 3);
        var machine = (MachineContent.MachineBlockEntity) level.getBlockEntity(pos);
        for (int i = 0; i < 8; i++) {
            machine.setRotation(i);
            h.assertTrue(
                    level.getBlockState(pos).getValue(MachineContent.ROTATION) == i,
    ...
    public static void observation_discovers_only_a_seen_active_complete_sign(GameTestHelper h) {
        ...
        player.setItemInHand(
                InteractionHand.MAIN_HAND, new ItemStack(CatalogItems.HAND_SPYGLASS.get()));
        h.runAfterDelay(
                2,
                () -> {
                    try {
    ...
    public static void visual_living_entities_have_attributes_and_spawn(GameTestHelper h) {
        var types=java.util.List.of(com.mpp.stellaeomphalos.content.entity.catalog.CatalogEntities.WISP.get(),
                com.mpp.stellaeomphalos.content.entity.catalog.CatalogEntities.LUMEN_DROPLET.get(),
                com.mpp.stellaeomphalos.content.entity.catalog.CatalogEntities.PHANTOM_TOOL.get());
        int i=0;
        for(var type:types){
