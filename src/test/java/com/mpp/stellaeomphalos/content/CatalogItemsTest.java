...
/**
 * 《方块物品实体完整清单》§6.4.4 / §6.4.5 物品持久数据的纯 JVM 验收。
 *
...
class CatalogItemsTest {
    ...
    void boon_seal_stores_and_reads_the_node_id() {
        var stack = new ItemStack(Items.PAPER);
        assertNull(CatalogItems.BoonSealItem.sealed(stack));
        var node = new ResourceLocation("stellaeomphalos", "key_bleed");
        CatalogItems.BoonSealItem.seal(stack, node);
        assertEquals(node, CatalogItems.BoonSealItem.sealed(stack));
    }
    ...
    void runed_wand_augments_round_trip_by_name() {
        var stack = new ItemStack(Items.STICK);
        assertEquals(
                com.mpp.stellaeomphalos.content.item.RodItems.WandAugment.NONE,
                com.mpp.stellaeomphalos.content.item.RodItems.augmentOf(stack),
                "A fresh wand has no augment");
        var wandAugment = com.mpp.stellaeomphalos.content.item.RodItems.WandAugment.PROSPECT;
        com.mpp.stellaeomphalos.content.item.RodItems.setAugment(stack, wandAugment);
        assertEquals(
                wandAugment,
                com.mpp.stellaeomphalos.content.item.RodItems.augmentOf(stack));
        ...
        stack.getOrCreateTag()
                .putString(
                        com.mpp.stellaeomphalos.content.item.RodItems.WandAugment.TAG,
                        "not_an_augment");
        assertEquals(
                com.mpp.stellaeomphalos.content.item.RodItems.WandAugment.NONE,
                com.mpp.stellaeomphalos.content.item.RodItems.augmentOf(stack));
        // 轮换顺序覆盖全部取值后回到起点。
        var current = com.mpp.stellaeomphalos.content.item.RodItems.WandAugment.NONE;
        for (int i = 0;
                i
                        < com.mpp.stellaeomphalos.content.item.RodItems.WandAugment
                                .values()
                                .length;
                i++) {
        ...
        assertEquals(
                com.mpp.stellaeomphalos.content.item.RodItems.WandAugment.NONE, current);
    }

    @Test
    void wand_charge_formula_matches_the_plan() {
        // §6.6.2：motion = look × clamp(tick/30, 0, 1) × 3，Y 分量额外保底 0.7×mul。
        assertEquals(30, com.mpp.stellaeomphalos.content.item.RodItems.RunedWandItem.CHARGE_TICKS);
        assertEquals(
                3.0F,
                com.mpp.stellaeomphalos.content.item.RodItems.RunedWandItem.CHARGE_SCALE);
        assertEquals(
                0.7D,
                com.mpp.stellaeomphalos.content.item.RodItems.RunedWandItem.CHARGE_Y_FLOOR);
        assertEquals(
                6000,
                com.mpp.stellaeomphalos.content.item.RodItems.LuminaryRodItem
                        .LUMINAIRE_BOOST_TICKS);
    }

    @Test
    void prospect_cache_is_bounded_and_clearable() {
        var cache = com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.class;
        assertNotNull(cache);
        var player = java.util.UUID.randomUUID();
        assertTrue(
                com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.last(player)
                        .isEmpty(),
                "Unknown player has no cached result");
        var many = new java.util.ArrayList<net.minecraft.core.BlockPos>();
        for (int i = 0;
                i
                        < com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache
                                        .MAX_ENTRIES
                                + 32;
                i++)
            many.add(new net.minecraft.core.BlockPos(i, 64, 0));
        com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.publish(player, many);
        assertEquals(
                com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.MAX_ENTRIES,
                com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.last(player)
                        .size(),
                "Prospect results must be truncated");
        com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.clear(player);
        assertTrue(
                com.mpp.stellaeomphalos.content.item.RodItems.WandProspectCache.last(player)
                        .isEmpty());
    }
    ...
    void builder_rod_plan_is_bounded_by_the_material_on_hand() {
        ...
        assertEquals(20, com.mpp.stellaeomphalos.content.item.RodItems.BuilderRodItem.MAX_PLACEMENT);
        assertEquals(
                5,
                com.mpp.stellaeomphalos.content.item.RodItems.SwapperRodItem.SEARCH_DEPTH);
        assertEquals(
                3.0D,
                com.mpp.stellaeomphalos.content.item.RodItems.SwapperRodItem
                        .DEFAULT_HARDNESS_LIMIT);
        assertTrue(
                com.mpp.stellaeomphalos.content.item.RodItems.BuilderRodItem.MAX_PLACEMENT
                        > com.mpp.stellaeomphalos.content.item.RodItems.SwapperRodItem
                                .SEARCH_DEPTH,
    ...
    void builder_rod_placement_record_carries_state_and_item() {
        ...
        var placement =
                new com.mpp.stellaeomphalos.content.item.RodItems.Placement(
    ...
    void luminary_rod_cycles_colours_by_name() {
        ...
        assertEquals(
                ...
                com.mpp.stellaeomphalos.content.item.RodItems.LuminaryRodItem.currentColor(
                        stack));
        var next =
                com.mpp.stellaeomphalos.content.item.RodItems.LuminaryRodItem.nextColor(stack);
        assertEquals(
                next,
                com.mpp.stellaeomphalos.content.item.RodItems.LuminaryRodItem.currentColor(
                        stack));
        stack.getOrCreateTag()
                .putString(
                        com.mpp.stellaeomphalos.content.item.RodItems.LuminaryRodItem.COLOR,
                        "bogus");
        assertEquals(
                net.minecraft.world.item.DyeColor.WHITE,
                com.mpp.stellaeomphalos.content.item.RodItems.LuminaryRodItem.currentColor(
